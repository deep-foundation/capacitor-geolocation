...
export async function makePositionInsertOperations<
  TDeepClient extends DeepClientInstance,
>(
  this: GeolocationDecorator<TDeepClient>,
  options: MakePositionInsertOperationsOptions,
): MakePositionInsertOperationsResult {
  const log = debug(
    `@deep-foundation/capacitor-geolocation:${makePositionInsertOperations.name}`,
  );
  log({ options });
  const { id } = options;
...
export type MakePositionInsertOperationsOptions = {
  position: Position | null;
  containerLinkId?: number;
  id?: number;
};
export type MakePositionInsertOperationsResult = Promise<
