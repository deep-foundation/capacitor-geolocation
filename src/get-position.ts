...
export async function getPosition<TDeepClient extends DeepClientInstance>(
  this: GeolocationDecorator<TDeepClient>,
  options: GetPositionOptions,
): GetPositionResult {
  const log = debug(
    `@deep-foundation/capacitor-geolocation:${getPosition.name}`,
  );
  log({ options });
  const {
    data: [positionLink],
  } = await this.select(options.linkId);
  log({ positionLink });
  if (!positionLink) {
  ...
  return {
    coords: {
      ...(minilinksCollection.query({
        id: { _id: [positionLink.id, "Coords", "Accuracy"] },
      })[0]?.value?.value ?? {}),
      ...(minilinksCollection.query({
        id: { _id: [positionLink.id, "Coords", "Altitude"] },
      })[0]?.value?.value ?? {}),
      ...(minilinksCollection.query({
        id: { _id: [positionLink.id, "Coords", "AltitudeAccuracy"] },
      })[0]?.value?.value ?? {}),
      ...(minilinksCollection.query({
        id: { _id: [positionLink.id, "Coords", "Heading"] },
      })[0]?.value?.value ?? {}),
      ...(minilinksCollection.query({
        id: { _id: [positionLink.id, "Coords", "Latitude"] },
      })[0]?.value?.value ?? {}),
      ...(minilinksCollection.query({
        id: { _id: [positionLink.id, "Coords", "Longitude"] },
      })[0]?.value?.value ?? {}),
      ...(minilinksCollection.query({
        id: { _id: [positionLink.id, "Coords", "Speed"] },
      })[0]?.value?.value ?? {}),
    },
    ...(minilinksCollection.query({
      id: { _id: [positionLink.id, "Timestamp"] },
    })[0]?.value?.value ?? {}),
  };
}
